...
function draw() {
  ...
  scrollText.forEach(scrolls => {
    ...
    topEase = easeTo(topEase, top, 10);
    if (top < 0) {
      scrolls.style.transform = `translateX(${50+topEase*.05}%)`;
...
document.addEventListener('scroll', function() {
  ...
  // console.log(map_container_mapped);
  if (map_container_mapped < max*2) {
      // path.style.setProperty('stroke-dashoffset', -map_container_mapped);
      // console.log(-map_container_mapped);
